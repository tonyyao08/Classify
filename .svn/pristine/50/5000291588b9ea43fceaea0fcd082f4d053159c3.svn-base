...
def classify_handler(image_path):
    ...
    # original image already uploaded, now we need to upload a flipped image to increase classification odds
    flipped_image_path = flip_image(image_name)
    flipped_image_name = flipped_image_path.rsplit('/', 1)[1]
    upload_to_s3(flipped_image_path, flipped_image_name)
...
def flip_image(image_path):
    ...
    flipped_image_path = image_path.split('.')[0] + "-flipped.jpg"
    vertical_img.save(flipped_image_path)
...
# HOW TO CALL IN CLASSIFY MODE: sample image_path for dev to use, change path
image_path = "C:/rekognition_backend/detect-text/wisconsin-quarter.jpg"
handler(image_path)